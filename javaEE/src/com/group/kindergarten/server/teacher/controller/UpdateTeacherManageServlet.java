 ...
 * 服务端修改教师信息
 */
@WebServlet("/updateTeacherManage")
public class UpdateTeacherManageServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateTeacherManageServlet() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (FileItem item : list) {
				if (item.isFormField()) {// 判断是否为文件
					if (item.getFieldName().equals("teacherId")) {
						String teacherId = item.getString();
		...
		boolean flag = teacherServiceImpl.updateTeacherService(teacher);
		if(flag) {
