...
public class UserParentDaoImpl {
	public static Connection connection;
	public static UserParentDaoImpl parentDao;
	 ...
	 * @return ParentDao
	 */
	public static UserParentDaoImpl getInstance() {
		if (null == parentDao) {
			parentDao = new UserParentDaoImpl();
		}
		if (null == connection) {
	...
	public boolean addParent(String phone, String nickname, String password) {
		...
		try {
			preparedStatement = connection
					.prepareStatement("insert into user_parent(phone,nickname,password) values (?,?,?)");
	...
	public boolean isExist(String phone) {
		String sql = "select * from user_parent where phone = ?";
		try {
	 ...
	 * @return Parent对象
	 */
	public UserParent selectOneParent(String phone) {
	 ...
	 * @return
	 */
	public List<UserParent> selectAllParent() {
		List<UserParent> parents=new ArrayList<UserParent>();
	...
	public List<UserParent> queryParentsByPhone(String query) {
		String sql = "select * from user_parent where phone=?";
		List<UserParent> parents = new ArrayList<UserParent>();
		try {
			preparedStatement = connection.prepareStatement(sql);
	...
	public Boolean updateParentMessage(String phone,String nickName,String headName) {
		boolean b=false;
		try {
			preparedStatement=connection.prepareStatement("update user set avatar=?,nickname=? where phone=?");
	...
	public static int countAll() {
		int count=0;
		try {
			preparedStatement =connection.prepareStatement("select count(id) from user_parent");
			ResultSet resultSet=preparedStatement.executeQuery();
			resultSet.next();
	 ...
	 */
	public static List<UserParent> selectPage(int pageNum, int pageSize) {
		List<UserParent> list=new ArrayList<UserParent>();
		try {
			preparedStatement=connection.prepareStatement("select * from user_parent limit ?,?");
	...
	public static int countAll(String searchInfo) {
		...
		String sql="select count(*) from user_parent where nickname like ? or phone like ?";
		try {
	...
	public static List<UserParent> selectPage(int pageNum, int pageSize, String searchInfo) {
		List<UserParent> list=new ArrayList<UserParent>();
		try {
			preparedStatement=connection.prepareStatement("select * from user_parent where nickname like ? or phone like ? limit ?,?");
			String str="%"+searchInfo+"%";
	...
	public String selectPasswordByPhone(String phone) {
		String password="";
		String sql="select password from user_parent where phone=?";
		try {
			preparedStatement=connection.prepareStatement(sql);
			preparedStatement.setString(1, phone);
			ResultSet resultSet=preparedStatement.executeQuery();
